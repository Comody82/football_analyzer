"""Dialog per preprocessing video - analisi automatica."""
...
from analysis.video_preprocessing import preprocess_video, get_preprocessed_path
...
class PreprocessWorker(QThread):
    ...
    def run(self):
        ...
        if ok:
            ...
            self.finished_signal.emit(False, "Errore durante il preprocessing.")


class VideoPreprocessingDialog(QDialog):
    """Dialog con barra progresso per preprocessing video."""

    def __init__(self, input_path: str, project_analysis_dir: str, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Preprocessing video - Analisi automatica")
        self.setFixedSize(440, 200)
        self.setWindowFlags(Qt.Dialog)
        self._input_path = input_path
        self._output_path = str(get_preprocessed_path(project_analysis_dir))
        self._worker = None
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        title = QLabel("PREPROCESSING VIDEO")
    ...
    def start(self):
        self._worker = PreprocessWorker(self._input_path, self._output_path)
    ...
    def _on_finished(self, ok: bool, message: str):
        ...
        if ok:
            QMessageBox.information(self, "Completato", f"Video preprocessato salvato in:\n{message}")
        else:
